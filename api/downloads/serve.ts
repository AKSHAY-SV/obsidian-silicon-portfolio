...
import { getFirestore, collection, query, where, getDocs, doc, getDoc, updateDoc, addDoc, serverTimestamp, Timestamp } from "firebase/firestore";
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const q = query(tokenCol, where("token", "==", downloadToken));
    ...
    const tokenDocRef = doc(db, "download_tokens", tokenDoc.id);
    ...
    if (isExpired || used || isLimitReached) {
      ...
      try {
        const reqDocRef = doc(db, "portfolio_access_requests", requestId);
